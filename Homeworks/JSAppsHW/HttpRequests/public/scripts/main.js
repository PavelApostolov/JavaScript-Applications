$(() => { // on document ready
    ...
    const GLYPH_DOWN = 'glyphicon-chevron-down';
    const root = $('#root');
    const navbar = root.find('nav.navbar');
    const mainNav = navbar.find('#main-nav');
    const contentContainer = root.find('#content');
    const loginForm = $('#login');
    const logoutForm = $('#logout');
    const usernameSpan = $('#span-username');
    const usernameInput = loginForm.find('input');
    const alertTemplate = $($('#alert-template').text());
    ...
    function loadThreadsContent() {
        ...
        function getThreadUI(thread) {
            var dateString = getDateTimeAgoString(thread.postDate);
    ...
    function getMsgUI(message) {
        var dateString = getDateTimeAgoString(message.postDate);
    ...
    function getDateTimeAgoString(date) {
        var momentDate = moment(date, 'YYYY-MM-DD HH:mm Z');
    ...
    contentContainer.on('click', '.btn-add-message', (ev) => {
        ...
        function appendMessage(result) {
            var last = result.messages.length - 1;
