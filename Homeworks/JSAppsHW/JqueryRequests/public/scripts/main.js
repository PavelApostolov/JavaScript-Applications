$(() => { // on document ready
  ...
  function loadMessagesContent(data) {
    let container = $($('#messages-container-template').text())
                    .attr('data-thread-id', data.result.id);
    if(contentContainer.find(`[data-thread-id="${data.result.id}"]`).length){
        container = contentContainer.find(`[data-thread-id="${data.result.id}"]`);
    }
    ...
    function getMsgUI(msg, author, date) {
      ...
      template.find('.message-creator').text(author&&author.username? author.username : 'anonymous');
    ...
    if(contentContainer.find(`[data-thread-id="${data.result.id}"]`).length===0){
  ...
  navbar.on('click', '#btn-threads', (ev) => {
    Promise.all([data.users.current(), data.threads.get()]).then(res => { 
      // if is logged in
      if(res[0]){
        loadThreadsContent(res[1].result)
      }else{
        showMsg('You are not logged in.', 'Authentication', 'alert-danger');
      }
    });
  });
  ...
  contentContainer.on('click', '#btn-add-thread', (ev) => {
    ...
    data.threads.add(title)
      .then(res=>{
        localThreads.push(res.result);
        loadThreadsContent(localThreads);
      })
  ...
  contentContainer.on('click', '.btn-add-message', (ev) => {
    ...
    data.threads.addMessage(thId, msg)
      .then(res => loadMessagesContent({ result: res }))
